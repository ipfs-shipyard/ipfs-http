  ...
  options: {
    handler: (request, reply) => {
      ...
      if (request.headers.accept === JSON) {
        promise = request.server.app.ipfs.dag.get(request.params.cid, request.query.path, {
    ...
    validate: {
      ...
      query: {
        ...
        path: Joi
          .string()
          .description('Path to resolve within this DAGNode'),
        offset: Joi
          .number()
          .integer()
          .min(0)
          .description('Byte offset to start streaming from'),
        length: Joi
          .number()
          .integer()
          .min(0)
          .description('Max number of bytes to stream'),
        localResolve: Joi
          .boolean()
          .default(false)
